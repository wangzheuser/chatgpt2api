...
class DropMailProvider(BaseMailProvider):
    name = "dropmail"
    # 仅保留本地实测可稳定注册 OpenAI 的 5 个域名（前 3 个 5/5，后 2 个 4/5）；
    # 其余 dropmail 域名邮箱能收码但被 OpenAI 注册黑名单拦截，故不纳入默认池。
    default_domains = ["mail2me.co", "pickmemail.com", "maximail.vip", "mailtowin.com", "pickmail.org"]
